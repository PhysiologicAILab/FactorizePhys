...
model_config = {
    ...
    "data_path": "/home/jitesh/data/iBVP_Dataset/iBVP_RGB_240_144x144/p07a_input1.npy",
    "label_path": "/home/jitesh/data/iBVP_Dataset/iBVP_RGB_240_144x144/p07a_label1.npy"
}
...
if __name__ == "__main__":
    ...
    if visualize:
        net = nn.DataParallel(FactorizePhysBig(frames=frames, md_config=md_config, device=device, in_channels=in_channels, debug=debug)).to(device)
        net.load_state_dict(torch.load(ckpt_path, map_location=device))
    else:
        net = FactorizePhysBig(frames=frames, md_config=md_config, device=device, in_channels=in_channels, debug=debug).to(device)
        net.eval()
